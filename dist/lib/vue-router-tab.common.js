...
function hasSlotContent(nodes) {
  return Array.isArray(nodes) && nodes.some(node => {
    if (!node || node.type === external_commonjs_vue_commonjs2_vue_root_Vue_namespaceObject.Comment) return false;
    if (node.type === external_commonjs_vue_commonjs2_vue_root_Vue_namespaceObject.Text) {
